...
import { useCreateClass } from "../api/useAcademic";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export const CreateClassModal = ({ isOpen, onOpenChange }: CreateClassModalProps) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || "Đã xảy ra lỗi khi tạo lớp học mới.";
      toast.error(msg);
    }
  };
      ...
      <DialogContent className="sm:max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl transition-colors">
        <DialogHeader className="space-y-2">
          ...
          <DialogDescription className="text-slate-500 dark:text-slate-400 text-xs">
            Nhập mã lớp học chính thức theo quy chuẩn đào tạo của trường.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="VD: SE1801, IA1702, SS1805"
                value={classCode}
