...
import { useDeleteClass } from "../api/useAcademic";
...
export const DeleteClassModal = ({ cls, isOpen, onOpenChange }: DeleteClassModalProps) => {
  const { mutateAsync: deleteClass, isPending } = useDeleteClass();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || "Đã xảy ra lỗi khi xóa lớp học.";
      toast.error(msg);
    }
  };
      ...
      <DialogContent className="sm:max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl transition-colors">
        <DialogHeader className="space-y-3">
          ...
          <DialogDescription className="text-slate-500 dark:text-slate-400 text-sm">
            Bạn có chắc chắn muốn xóa lớp học <span className="font-bold text-slate-900 dark:text-slate-100 font-mono">[{cls?.classCode}]</span> khỏi hệ thống? Hành động này không thể hoàn tác.
          </DialogDescription>
        </DialogHeader>
