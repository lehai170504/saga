...
import { useUpdateSubject } from "../api/useAcademic";
...
export const EditSubjectModal = ({ subject, isOpen, onOpenChange }: EditSubjectModalProps) => {
  const [subjectCode, setSubjectCode] = useState("");
  const [subjectName, setSubjectName] = useState("");

  const { mutateAsync: updateSubject, isPending } = useUpdateSubject();

  useEffect(() => {
    if (subject) {
      setSubjectCode(subject.subjectCode || subject.code || "");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || "Đã xảy ra lỗi khi cập nhật môn học.";
      toast.error(msg);
    }
  };
      ...
      <DialogContent className="sm:max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl transition-colors">
        <DialogHeader className="space-y-2">
          ...
          <DialogDescription className="text-slate-500 dark:text-slate-400 text-xs">
            Cập nhật lại mã môn học và tên hiển thị chính thức.
          </DialogDescription>
        </DialogHeader>
