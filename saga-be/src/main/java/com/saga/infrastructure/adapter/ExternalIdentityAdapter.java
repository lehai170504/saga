...
@Component
public class ExternalIdentityAdapter implements ExternalIdentityPort {
    ...
    @Override
    public com.saga.application.port.ExternalUserProfile getGithubProfile(String code) {
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(List.of(MediaType.APPLICATION_JSON));
        Map<String, String> request = Map.of("client_id", githubClientId, "client_secret", githubClientSecret, "code",
                code);
        HttpEntity<Map<String, String>> entity = new HttpEntity<>(request, headers);
        ResponseEntity<Map> tokenResponse = restTemplate.postForEntity("https://github.com/login/oauth/access_token",
                entity, Map.class);
        ...
        String email = emailsResponse.getBody().stream().map(m -> (String) m.get("email")).findFirst().orElse("");
        ResponseEntity<Map> userResponse = restTemplate.exchange("https://api.github.com/user", HttpMethod.GET, new HttpEntity<>(emailHeaders), Map.class);
        String name = (String) userResponse.getBody().get("name");
        if (name == null || name.isEmpty()) name = (String) userResponse.getBody().get("login");
        String id = String.valueOf(userResponse.getBody().get("id"));
        return com.saga.application.port.ExternalUserProfile.builder().id(id).name(name).email(email).build();
    }

    @Override
    public com.saga.application.port.ExternalUserProfile getJiraProfile(String code) {
        HttpHeaders headers = new HttpHeaders();
